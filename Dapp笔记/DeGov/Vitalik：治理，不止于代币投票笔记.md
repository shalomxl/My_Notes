...
- 我们需要超越现有的代币投票形式的DeGov。

## 正文

### DeGov 是有必要的

赛博朋克的价值观：使用密码学来最小化集权控制，维护私有财产，扩大市场范围。通过非对称加密，来保证私有财产的不可侵犯性。通过智能合约降低人们之间的合作门槛，信任更加容易产生。

上述的活动形式是一对一的，然而在许多领域中，存在着多人互动的场景，例如：艺术、撰写文档、科学研究和程序编码。在这些一对多或者多对多的场景中，就需要DeGov。
...
V神主张尽量减少所有非自动化的参数调整，并且他是 RAI协议的治理策略 的忠实粉丝（随着时间推移，治理对功能的控制会越来越少），但是有些情况必须要社区人为的介入：

- 价格预言机输入必须要有来源，而这个来源有时需要改变；
- 如果美元崩盘，RAI 不得不争先恐后地创造以及维持它们自己去中心化的 CPI 指数；

在一个协议完全进入稳定期之前，DeGov是完全有必要的。

##### 应用层无法避免链上治理

事实上，对于基础层区块链来说，链下治理是完全可能的。但是对于应用层的项目 (尤其是 DeFi 项目) 来说，我们会遇到这样的问题：应用层智能合约系统往往直接控制外部资产，并且这种控制无法被分叉。治理合约被攻击，被盗走的资金无法通过分叉挽回（这里不是指所有的合约攻击事件，而是特指治理合约攻击）。
...
1. 用户用“声望”投票，这是一种无法转移的代币。如果用户的决策带来理想结果，他们会获得声誉，如果他们的决策带来不理想的结果，则会失去声誉；[声誉VS代币](https://medium.com/daostack/reputation-vs-tokens-6d7642c7a538)
2. 不会直接实现提议变更的代币投票，相反，它的存在只是为了公示结果，为链下治理建立[合法性 (legitimacy)](https://mp.weixin.qq.com/s/7BPTa0_zNzTooeM7ZTMnJQ)，以执行该变更。
3. 时间延迟+选举专家治理：若干个专家发起提案，投票选举选用哪个提案，投票的规则是：每周淘汰一个提案，最终留下来的提案为成功提案。用户可以有很长的时间发现不信任的提案并淘汰掉它。
